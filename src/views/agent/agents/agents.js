...
const Agents = ({ agents, updateAgent }) => {
  ...
  const filterAgents = (agents, selectedType, filter) => {
    return agents.filter(agent => {
      const filterResult = JSON.stringify(agent).indexOf(filter) > -1
      if (!filterResult) {
        return filterResult
      }
      if (selectedType === 'All') {
      ...
      return agent.type.toLowerCase() === selectedType.toLowerCase()
    })
  }

  const { selectedType, filter } = useContext(FilterTypeCtx)

  return (
    <div className='agents'>
      {arrayNotEmpty(agents) &&
        filterAgents(agents, selectedType, filter).map(agent => (
